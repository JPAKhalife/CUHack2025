...
export const FixIconUrl = `data:image/svg+xml;base64,${btoa(`<svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
...
export const overlayTemplate = (overlayStyle)=>{
    return /* html */ `
<style>
  ${Style(overlayStyle)}
</style>
...
<div class="error-overlay">
  <div class="error-container">
    <div class="error-content">
      ...
      ${overlayStyle.style === "backendError" ? /* html */ `
        ...
        ` : overlayStyle.style === "viteFrontend" ? /* html */ `
...
<script defer suppressHydrationWarning>
  ...
  const fixErrorLink = \`\${window.location.origin}/edit/${overlayStyle.environmentSlug}?prompt=${encodeURIComponent("Can you fix the following error?\n\n" + getErrorMessage(overlayStyle)).slice(0, 10000)}\`;
    ...
    constructor(err){
        ...
        if (fixButton) {
            fixButton.addEventListener("click", ()=>{
                if (window.self !== window.top) {
                    window.parent.postMessage({
                        type: "gadget:fixError",
                        error: {
                            errorMessage: err.message
                        }
                    }, "*");
                } else {
                    window.open(`${window.location.origin}/edit?prompt=${encodeURIComponent(`Can you fix the following error?\n\n${err.message}`)}`, "_blank");
                }
            });
        } else {
