...
export function buildComputedView<VariablesT, ResultT>(
  ...
  const f = operation.variables
    ? async (variables: Record<string, any> = {}) => {
        ...
        return await computedViewRunner(client.connection, operation.gqlFieldName, resultVariables, operation.namespace);
      }
    : async () => {
  ...
  return Object.assign(f, operation, {
    plan: function (variables: VariablesOptions) {
      return computedViewOperation(operation.operationName, variables, operation.namespace);
    },
  }) as any;
}
